...
class PageGetter(Thread):
    ...
    def get_pages_cycle(self):
        ...
        for n, art in enumerate(self.links):
            ...
            write_html(self.browser.page_source, f'{self.html_dir}{self.date}_N{order:04}_{self.platform}_{art}.html')
...
class GetterOz(PageGetter):
    ...
    def scroll_down(self):
        ...
        for i in range(1, 5):
            new_height = self.browser.execute_script("return document.body.scrollHeight")
            self.browser.execute_script(f"window.scrollTo(0, {new_height - 500 * i});")
            time.sleep(sl_time)
            self.browser.execute_script(f"window.scrollTo(0, {new_height + 500 * i});")
            time.sleep(sl_time)
        while True:
